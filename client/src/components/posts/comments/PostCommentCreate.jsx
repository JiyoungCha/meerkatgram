...
export default function PostCommentCreate({postId, replyId = 0}) {
  ...
  async function storeComment() {    
    ...
    await dispatch(postStoreThunk());
  }

  return (
    <>
      <form className="post-comment-create-box" onSubmit={handleCommentStore}>
        <input type="text" className="post-comment-create-input-add" onChange={(e) => { setContent(e.target.value) }} placeholder={`${postId} ${replyId} add comments...`} />
